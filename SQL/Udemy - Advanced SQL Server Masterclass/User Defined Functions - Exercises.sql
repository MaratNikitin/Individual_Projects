...
CREATE OR ALTER FUNCTION dbo.fnFirstNumberAsPercentOfSecond(@FirstNumber INT, @SecondNumber INT)
RETURNS VARCHAR(255)
AS
BEGIN
	RETURN FORMAT((@FirstNumber * 1.0 / @SecondNumber), 'P')
END
GO

-- Test the new function:
SELECT dbo.fnFirstNumberAsPercentOfSecond(8,11) AS Result

-- Marat Nikitin: by the way, it's not a good idea to create a user defined function for something as simple as this from the performance point of view. 
...
JobTitle
VacationHours
Then add a derived field called "PercentOfMaxVacation", which returns the percent an individual employees' vacation hours 
...
The derived field should make use of your user-defined function from the previous exercise, as well as your variable 
that stored the maximum vacation hours for any employee.
*/

DECLARE @MaximumVacationHours INT

SELECT @MaximumVacationHours = MAX(VacationHours) FROM AdventureWorks2019.HumanResources.Employee

SELECT 
BusinessEntityID,
JobTitle,
VacationHours,
PercentOfMaxVacation = dbo.fnFirstNumberAsPercentOfSecond(VacationHours, @MaximumVacationHours)
