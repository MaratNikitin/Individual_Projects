...
@NameToSearch: The user will be expected to enter either "first", "middle", or "last". This way, they 
do not have to remember exact column names.
...
pattern anywhere within it.
I.e., if the user tells us to search the FirstName field for the pattern "ravi", both the names 
"Ravi" and "Travis" should be returned.
Hints:
You will probably want to use LIKE with a wildcard in your WHERE clause.
To include single quotes in your dynamic SQL, try "escaping" them by typing four consecutive single 
quotes ('''').
Try creating a variable to hold the actual column name to search, and then set this variable using 
IF statements, based on the value passed into the "NameToSearch" parameter by the user. Then simply 
plug this variable into your dynamic SQL. This is easier than having to execute different queries 
...
/*
Exercise 2
Modify your "NameSearch" procedure to accept a third argument - @MatchType, with an INT datatype -  
that specifies the match type:
1 means "exact match"
2 means "begins with"
3 means "ends with"
4 means "contains"
Hint: Use a series of IF statements to build out your WHERE clause based on the @MatchType parameter, 
then append this to the rest of your dynamic SQL before executing.
*/
