...
"Price Rank " that ranks all records in the dataset by ListPrice, in descending order. That is to say, the product with the most 
	expensive price should have a rank of 1, and the product with the least expensive price should have a rank equal to the number 
...
SELECT p.[Name] AS ProductName, 
	...
	pc.[Name] AS ProductCategory,
	PriceRank = ROW_NUMBER() OVER(ORDER BY p.ListPrice DESC),
	CategoryPriceRank = ROW_NUMBER() OVER(PARTITION BY pc.[Name] ORDER BY p.ListPrice DESC)
FROM [AdventureWorks2019].[Production].[Product] p
	JOIN [AdventureWorks2019].[Production].[ProductSubcategory] ps ON p.ProductSubcategoryID = ps.ProductSubcategoryID
	JOIN [AdventureWorks2019].[Production].[ProductCategory] pc ON pc.ProductCategoryID = ps.ProductCategoryID
...
-- The solution for Exercise 4 suggested by the exercise's author:
